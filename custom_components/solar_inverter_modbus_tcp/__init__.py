...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    coordinator = SolarInverterCoordinator(
        ...
        ev_charger_rated_power_kw=entry.options.get(
            CONF_EV_CHARGER_RATED_POWER_KW,
            entry.options.get("ev_charger_1_rated_power_kw", 11.0),
        ),
    )
    ...
    _async_migrate_ev_charger_control_entities(hass, coordinator)
    _async_sync_optional_sensor_entities(hass, coordinator)
    _async_disable_second_ev_charger_entities(hass, coordinator)
...
def _async_migrate_sensor_entity_ids(
    ...
    for charger, descriptions in ((1, EV_CHARGER_DESCRIPTION),):
        for description in descriptions:
            suffix = slugify(description.name.removeprefix("EV Charger "))
            migrations.append(
                (
                    f"{DOMAIN}_{coordinator.entry_id}_{description.key}",
                    f"sensor.ev_charger_{suffix}",
                )
            )
...
def _async_migrate_ev_charger_control_entities(
    hass: HomeAssistant, coordinator: SolarInverterCoordinator
) -> None:
    """Move EV Charger 1 controls to their single-charger identities."""
    registry = er.async_get(hass)
    for platform, data_key in (
        ("number", "charging_power_setting"),
        ("number", "offline_charging_power"),
        ("number", "max_charging_power_from_grid"),
        ("select", "charging_mode"),
        ("select", "green_power_mode"),
    ):
        old_unique_id = f"{DOMAIN}_{coordinator.entry_id}_ev_charger_1_{data_key}"
        new_unique_id = f"{DOMAIN}_{coordinator.entry_id}_ev_charger_{data_key}"
        entity_id = registry.async_get_entity_id(platform, DOMAIN, old_unique_id)
        if (
            entity_id is None
            or registry.async_get_entity_id(platform, DOMAIN, new_unique_id)
            is not None
            or registry.async_get(f"{platform}.ev_charger_{data_key}") is not None
        ):
            continue
        registry.async_update_entity(
            entity_id,
            new_unique_id=new_unique_id,
            new_entity_id=f"{platform}.ev_charger_{data_key}",
        )


def _async_disable_second_ev_charger_entities(
    hass: HomeAssistant, coordinator: SolarInverterCoordinator
) -> None:
    """Disable entities from the retired second EV charger."""
    registry = er.async_get(hass)
    for address in range(3250, 3275):
        unique_id = f"{DOMAIN}_{coordinator.entry_id}_r{address}"
        entity_id = registry.async_get_entity_id("sensor", DOMAIN, unique_id)
        if entity_id is None:
            continue
        registry_entry = registry.async_get(entity_id)
    ...
    for platform, data_key in (
        ("number", "ev_charger_2_charging_power_setting"),
        ("number", "ev_charger_2_offline_charging_power"),
        ("number", "ev_charger_2_max_charging_power_from_grid"),
        ("select", "ev_charger_2_charging_mode"),
        ("select", "ev_charger_2_green_power_mode"),
    ):
        unique_id = f"{DOMAIN}_{coordinator.entry_id}_{data_key}"
